...
import time
time.time() # 获取时间戳
time.strftime('%Y-%m-%d %M:%S %p') # 格式化显示时间
time.localtime() # 结构化时间 --单独获取时间的某一部分

import datetime
datetime.datetime.now() # 获取当前时间
datetime.datetime.now() + datetime.timedelta(days=3) # 获取三天后的时间

# 时间格式转换
time.mktime()  # 结构化 --> 时间戳
time.localtime() # 时间戳 --> 结构化

time.gmtime() # 世界标准时间

time.strftime('格式化形式'，时间) # 结构化 --> 字符串时间
time.strptime('时间字符串'，格式化形式) # 字符串时间 --> 结构化

time.sleep()
time.asctime() 
datetime.utcnow()
datetime.formtimestamp()
```
...
```python
import random
random.random(start_num, end_num) # []
random.randint(start_num, end_num) # []
random.randrange(start_num, end_num) # [)
random.choice([]) # 任意取一个
random.sample([], num) # 任意取n个
random.uniform(start_num, end_num) # 取小数
...
os.makedirs() # 创建文件夹(可递归 生成多级目录)
os.removedirs('dirname') # 目录为空则删除 并递归到上一级目录
os.mkdir('dirname') # 生成单级目录
os.rmdir('dirname') # 删除单级空目录
os.listdir(dirname) # 列出当前文件夹下的子文件 子文件夹
os.remove() # 删除一个文件
os.rename('oldname', 'newname') # 重命名
os.stat('path/filename') # 获取文件目录的位置
os.sep # 路径分割符
os.linesep() # 输出当前平台使用的终止符
os.pathsep() # 输出用于分隔文件路径的字符串符号
os.name # 输出字符串指示当前使用平台
os.system() # 运行系统命令
os.environ # 获取系统环境变量
os.path.abspath(path) # 返回绝对路径
os.path.split(path) # 将目录与文件名分割 以二元元组的形式返回
os.path.dirname(path) # 返回path的目录
os.path.basename(path) # 返回path的文件名
os.path.isabs(path) # 判断是否为绝对路径
os.path.isfile(path) # 判断是否为文件
os.path.isdir(path) # 判断是否存在这个目录
os.path.join() # 获取绝对路径
os.path.getatime(path) # 返回path所指向的文件或目录
os.path.getntime(path) # 返回path所指向的文件或目录
...
```python
sys.path() # 获取程序当前路径
sys.argv # 在python程序外 向python传递参数 在程序内获取到一个参数列表
```
